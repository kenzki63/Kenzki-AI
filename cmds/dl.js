const { kenzki, wtype, prefix, sleep, extractUrlsFromString, fb, ytaudio, config, ytvideo, xdl, tt, insta, mediaFire, rand, getBuffer } = require("../core")
...
cmd: "apk",
  ...
  try {
    ...
    var data = await m.axios(`https://api.kenzki.live/api/apk?q=${text}`)
...
cmd: "apksearch",
  ...
  try {
    ...
    var data = await m.axios(`https://api.kenzki.live/api/apkdl?id=${q}`)
    if (data.error) return await m.send("_Apk not found.._")
    ...
    var info = await m.axios(`https://api.kenzki.live/api/apksearch?query=${text}`);
...
kenzki({
  ...
  let data;
  try {
    data = await m.axios(`https://api.kenzki.live/api/subtitle?q=${text}`);
  } catch (e) {
...
cmd: "subtitlesearch|subtitles",
  ...
  try {
    ...
    let data = await m.axios(`https://api.kenzki.live/api/subtiledl?q=${encodeURIComponent(pageUrl)}`);
    ...
    const info = await m.axios(`https://api.kenzki.live/api/subtitlepage?q=${text}`);
...
kenzki({
        ...
        try {
                ...
                const ttregex = /https:\/\/(?:www\.|vm\.|m\.|vt\.)?tiktok\.com\/(?:(@[\w.-]+\/(?:video|photo)\/\d+)|v\/\d+\.html|[\w-]+\/?)(?:\?.*)?$/
                var link = links.find(url => ttregex.test(url));
                const dta = await fetch(`https://api.kenzki.live/api/tik-img?url=${encodeURIComponent(link)}`);
...
kenzki({
        ...
        try {
                ...
                const api = `https://api.kenzki.live/api/pinterest?url=${encodeURIComponent(link)}`
