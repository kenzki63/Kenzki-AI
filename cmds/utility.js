 ...
 * This file is part of kenzki and is licensed under the GNU GPLv3.
...
const { kenzki, extractUrlsFromString, getJson, Baileys, talkNote, prefix, wtype, config, ss } = require("../core")
const fs = require("fs");
...
cmd: "url|tourl|upload",
        desc: "uploads quoted media to kenzki\'s Cdn and sends access url(safe)",
        fromMe: wtype,
        react: "🔗",
        type: "utilities",
}, async (m, text) => {
...
cmd: "temp-url|temp-upload",
        desc: "uploads quoted media to kenzki\'s Cdn and sends access url(temporarily)",
        fromMe: wtype,
        react: "♻️",
        type: "utilities",
}, async (m, text) => {
...
kenzki({
    ...
    try {
        ...
        var res = await m.axios(
            `https://api.kenzki.live/api/translate?text=${encodeURIComponent(t)}&to=${code}`
        )
...
cmd: "ngl",
  ...
  try {
    ...
    var res = await m.axios(`https://kenzki-api.vercel.app/ngl?username=${encodeURIComponent(user)}&message=${encodeURIComponent(msg)}`)
    if (res?.success) {
