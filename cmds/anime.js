...
kenzki(
  ...
  async (m, text) => {
    try {
      ...
      return m.send(
        json.url,
        { caption: `> here's your ${category} waifu` },
        "image",
      );
    } catch (e) {
      console.log("cmd error", e);
  ...
  async (m, text) => {
    try {
      ...
      if (data.data.length < 1)
        return m.send("_*no manga found with that name!*_");
  ...
  async (m, text) => {
    try {
      ...
      if (!data.data || data.data.length < 1)
        return m.send("_*no trending anime found!*_");
...
kenzki(
  ...
  async (m, text) => {
    try {
      ...
      m.react("🔍");
      const res = await fetch(
        `https://api.jikan.moe/v4/characters?q=${encodeURIComponent(text)}&limit=1`,
      );
      const json = await res.json();

      if (json.data.length < 1)
        return m.send("_*no character found with that name!*_");
  ...
  async (m) => {
    try {
      if (!m.quoted || !m.quoted.media)
        return m.send("_*reply to anime image to search*_");
      m.react("⏳");
      ...
      if (!result.result || result.result.length < 1)
        return m.send("_*couldn't identify anime from image*_");

      const match = result.result[0];
      const preview = await fetch(
        `https://media.trace.moe/video/${match.anilist}/${encodeURIComponent(match.filename)}?t=${match.from}&token=${match.video}`,
      );
...
kenzki(
  ...
  async (m, text) => {
    ...
    try {
      ...
      if (!text) {
        if (watchlist[m.sender].length === 0)
          return m.send(
      ...
      if (parts.length < 2)
        return m.send("_*format:*_ .animewatch <anime name> | <episode>");

      const animeName = parts[0];
      const episode = parts[1];

      const existingIndex = watchlist[m.sender].findIndex(
      ...
      if (existingIndex !== -1) {
        ...
        fs.writeFileSync(watchlistFile, JSON.stringify(watchlist, null, 2));
        return m.send(
          `_*added ${animeName} (episode ${episode}) to watchlist*_ ✅`,
        );
      }
    } catch (e) {
      console.log("cmd error", e);
  ...
  async (m, text) => {
    try {
      ...
      if (!json.results || json.results.length < 1)
        return m.send("_*no gif found*_ 😔");

      return m.send(
        json.results[0].url,
        { caption: `> ${category} anime gif`, gifPlayback: true },
        "video",
      );
    } catch (e) {
      console.log("cmd error", e);
...
kenzki(
  ...
  async (m, text) => {
    try {
      if (!text)
        return m.send(
          "_*provide season info:*_ .season <year> <season>\nSeasons: winter, spring, summer, fall",
        );
      m.react("🔍");
      ...
      if (!validSeasons.includes(season))
        return m.send(
          "_*invalid season! Choose:*_ winter, spring, summer, fall",
        );
      if (isNaN(year) || year < 1990 || year > 2030)
        return m.send("_*provide valid year between 1990-2030*_");

      const res = await fetch(
        `https://api.jikan.moe/v4/seasons/${year}/${season}?limit=10`,
      );
      const json = await res.json();

      if (!json.data || json.data.length < 1)
        return m.send(`_*no anime found for ${season} ${year}*_`);
...
kenzki(
  ...
  async (m, text) => {
    try {
      ...
      if (!json.data || json.data.length < 1)
        return m.send("_*no recommendations found*_ 😔");

      let msg = text
        ? `🎬 *BECAUSE YOU LIKE ${text.toUpperCase()}* 🎬\n\n`
...
kenzki(
  ...
  async (m, text) => {
    try {
      ...
      const res = await fetch(
        `https://api.jikan.moe/v4/anime?q=${encodeURIComponent(text)}&status=airing&limit=1`,
      );
      const json = await res.json();

      if (!json.data || json.data.length < 1)
        return m.send(`_*couldn't find airing info for ${text}*_`);

      const anime = json.data[0];
      if (anime.status !== "Currently Airing")
        return m.send(`_*${anime.title} is not currently airing*_`);

      const schedule = await fetch(
        `https://api.jikan.moe/v4/anime/${anime.mal_id}/schedule`,
      );
      const scheduleJson = await schedule.json();

      if (!scheduleJson.data || Object.keys(scheduleJson.data).length === 0)
        return m.send(
          `_*no schedule information available for ${anime.title}*_`,
        );
      ...
      if (!nextEp)
        return m.send(`_*no upcoming episodes found for ${anime.title}*_`);
